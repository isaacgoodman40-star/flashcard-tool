    ...
    for eid in email_ids:
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                results.append({
                    "id": str(eid), "from": sender, "subject": subject,
                    "date": date_str, "category": category, "reason": reason,
...
with st.expander("📋 Important — Read First!", expanded=True):
    ...
    1. **Gmail**: Create an **App Password** (2-Step Verification ON first) → Google Account → Security → App Passwords
...
if pin_input == ADMIN_PIN:
    ...
    st.info("Inbox Cleaner — Free Edition with Support Option")
